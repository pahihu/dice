...
int32_t STAR32(void *_ptr)
{
    unsigned char *ptr = _ptr;
    return (((((ptr[0]<<8)+ptr[1])<<8)+ptr[2])<<8)+ptr[3];
}

uword STAR16(void *_ptr)
{
    unsigned char *ptr = _ptr;
    return (ptr[0]<<8)+ptr[1];
}

#define BIG16(x)        ((((x)&255)<<8)+((x)>>8))
#else
#define STAR32(x)       (*(x))
#define STAR16(x)       (*(x))
#define BIG16(x)        (x)
...
main(int ac, char **av)
{
	...
	while (scan < bend && STAR32(scan) != 0x3E7) {
	    uint32_t len;

	    switch((uword)STAR32(scan)) {
		...
		break;
	    default:
		printf("Unknown hunk type $%08x", STAR32(scan));
		exit(1);
	    }
	}
    }
...
int len;
{
	...
	case 0x2079:	/*  move.l  abslong,A0	*/
	    tscan[0] = BIG16(0x206C);
	    tscan[1] = tscan[2];
	    tscan[2] = BIG16(0x4E71);
	    break;
	case 0x2C79:	/*  move.l  abslong,A6	*/
	    tscan[0] = BIG16(0x2C6C);
	    tscan[1] = tscan[2];
	    tscan[2] = BIG16(0x4E71);
	    break;
	case 0xD1F9:	/*  adda.l  abslong,A0	*/
	    tscan[0] = BIG16(0xD1EC);
	    tscan[1] = tscan[2];
	    tscan[2] = BIG16(0x4E71);
	    break;
	default:
