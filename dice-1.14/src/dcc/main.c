...
void uexit(int code);
void DicePrefix(char **pbuf, char *av0, char *app);
...
myexit()
{
    ...
    while ((node = RemHead(&TmpList)) != NULL) {
	remove(node->ln_Name);
	free(node);
    }
    if (ErrFileIsTmp) {
...
char *xav[];
{
	...
	DicePrefix(&DLINK, av[0], "dlink");
	DicePrefix(&DAS, av[0], "das");
	DicePrefix(&DC1, av[0], "dc1");
	DicePrefix(&DCPP, av[0], "dcpp");
	...
	if (i > ac) {
	    eprintf("DCC: file argument missing\n");
	    help(1);
	}
    }
...
DoPrelink_Dice(void)
{
    ...
    while ((nn = (NameNode *)RemHead(&LList)) != NULL) {
	fputs(nn->n_In, fi);
	putc('\n', fi);
    }
